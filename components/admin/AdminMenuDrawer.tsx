...
export function AdminMenuDrawer({
  ...
  return (
    <>
      ...
      {/* Drawer Panel */}
      <div
        className={`fixed top-0 left-0 h-full w-64 bg-muted border-r border-border flex flex-col z-50 transition-transform duration-300 ease-out ${
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={onClose}
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-muted-foreground hover:bg-card hover:text-white transition"
            >
