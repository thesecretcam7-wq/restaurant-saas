...
function RegisterForm() {
  ...
  // Pre-fill email from OAuth callback
  useEffect(() => {
    const email = searchParams.get('email')
    const oauth = searchParams.get('from_oauth') === 'true'
    if (email) {
      setForm(f => ({ ...f, email }))
      setFromOAuth(oauth)
    }
            ...
            <div className="rounded-xl border border-border bg-card/50 backdrop-blur-sm p-8 shadow-md">
              ...
              {fromOAuth && (
                <div className="mb-6 flex items-start gap-3 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-blue-600 flex-shrink-0 mt-0.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
                  <p className="text-blue-600 text-sm font-medium">Completa los datos de tu restaurante para continuar</p>
                </div>
              )}
