...
const STATUS: Record<string, { label: string; bg: string; dot: string; icon: string }> = {
  ...
  confirmed:  { label: 'Confirmado',   bg: '#DBEAFE', dot: '#4F46E5', icon: '✅' },
      ...
      <main className="max-w-lg mx-auto px-4 py-5 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ backgroundColor: 'var(--primary-color, #4F46E5)15', color: primary }}>
              📋
            </div>
            <div>
                            ...
                            <div key={s} className="flex items-center flex-1 last:flex-none">
                              <div className="flex flex-col items-center gap-1">
                                <div
                                  ...
                                  style={{ backgroundColor: i <= currentStep ? 'var(--primary-color, #4F46E5)' : '#E2E8F0' }}
                                />
                                <span className="text-[8px] text-muted-foreground font-medium whitespace-nowrap">{STEP_LABELS[i]}</span>
                              </div>
                              {i < STEPS.length - 1 && (
                                <div className="flex-1 h-0.5 mb-3 mx-0.5 transition-all" style={{ backgroundColor: i < currentStep ? 'var(--primary-color, #4F46E5)' : '#E2E8F0' }} />
                              )}
                            </div>
                          ))}
                        </div>
