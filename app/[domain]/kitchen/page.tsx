...
import { ChefHat, Plus, Minus, Trash2, Send, Search, CheckCircle, X } from 'lucide-react';
...
export default function KitchenPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setTenantName(tenant.organization_name);

      const [{ data: cats }, { data: items }] = await Promise.all([
        supabase.from('menu_categories').select('id, name, sort_order').eq('tenant_id', tenant.id).eq('active', true).order('sort_order'),
        supabase.from('menu_items').select('id, name, price, category_id, description, image_url').eq('tenant_id', tenant.id).eq('available', true),
      ]);
  ...
  const sendOrder = async () => {
    if (!tenantId || cart.length === 0 || !tableNumber) return;
    setSending(true);
    ...
    try {
      ...
      // Create order_items so they appear in KDS
            ...
            {cart.length === 0 ? (
              ...
              <div key={item.menu_item_id} className="bg-gray-800 rounded-xl p-3">
                ...
                {editingNote === item.menu_item_id ? (
                  ...
                  <button onClick={() => { setEditingNote(item.menu_item_id); setNoteText(item.notes); }} className="mt-1.5 text-xs text-gray-500 hover:text-gray-300 flex items-center gap-1">
